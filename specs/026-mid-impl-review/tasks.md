...
- [x] T001 Read existing files: `spex/extensions/spex/commands/speckit.spex.ship.md` (Stage 6 implement subagent prompt), `spex/extensions/spex-deep-review/commands/speckit.spex-deep-review.run.md` (deep review command), `spex/scripts/spex-ship-state.sh` (state management)
...
- [x] T002 [US1] Add `checkpoint-record` command to `spex/scripts/spex-ship-state.sh`: accepts `--checkpoint <1|2> --findings <N> --fixed <N>`, writes `checkpoint_N_findings` and `checkpoint_N_fixed` to the state file. If the state file doesn't exist, create a minimal one. Update the script's usage comment to include the new command
...
- [x] T007 [US2] Add MVP designation logic: after the statistics table, identify the agent with the highest "Found" count and output "MVP: {agent name} ({count} findings)". If all agents found 0, output "Clean review: no findings across {N} agents"
- [x] T008 [US2] Add a total row to the statistics table summing Found, Fixed, and Remaining across all agents
...
- [x] T009 [US3] In `speckit.spex-deep-review.run.md`, after the agent leaderboard, check the state file for `checkpoint_1_findings` and `checkpoint_2_findings`. If present, output a layer comparison table with columns: Layer, Findings, Fixed, Unique
- [x] T010 [US3] Implement the "unique" calculation: for each layer, compare its finding locations (file path + line range) against all other layers' findings. A finding is "unique" if no other layer reported a finding at the same file path with overlapping lines. If line numbers are unavailable, fall back to finding description substring match
- [x] T011 [US3] When no checkpoint data exists in the state file (regular flow or checkpoints disabled), skip the layer comparison entirely. Only show the agent leaderboard
...
- [x] T012 [P] Update `README.md`: add mid-implementation review checkpoints description to the ship pipeline section, mention agent leaderboard in the deep review extension description
- [x] T013 [P] Update `spex/docs/help.md`: add `implement.review_checkpoints` config key alongside existing `implement.test_between_tasks`
