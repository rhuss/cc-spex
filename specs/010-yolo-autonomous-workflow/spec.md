# Feature Specification: Autonomous Full-Cycle Workflow (spex:yolo)
...
A developer has completed a brainstorming session and has a brainstorm document in `brainstorm/`. They invoke `/spex:yolo` to run the entire spex workflow autonomously, from specification through implementation and review, without manually triggering each stage.
...
**Independent Test**: Can be tested by creating a brainstorm document and running `/spex:yolo brainstorm/05-test-feature.md`. The skill should execute specify, clarify, review-spec, plan, review-plan, tasks, implement, deep-review, and verify in sequence, producing a working implementation.

**Acceptance Scenarios**:

1. **Given** a brainstorm document exists at `brainstorm/05-yolo-autonomous-workflow.md` and both `superpowers` and `deep-review` traits are enabled, **When** the user runs `/spex:yolo brainstorm/05-yolo-autonomous-workflow.md`, **Then** the skill executes the full pipeline: specify, clarify, review-spec, plan, review-plan, tasks, implement, deep-review, verify.
2. **Given** the pipeline is running in `balanced` autonomy mode, **When** a review stage finds a clear formatting issue, **Then** the skill auto-fixes it and re-runs the review without stopping.
3. **Given** the pipeline is running in `balanced` autonomy mode, **When** a review stage finds an ambiguous architectural issue, **Then** the skill pauses and presents the finding to the user for guidance.
4. **Given** the user answers a blocking question, **Then** the pipeline automatically resumes from where it paused.

---

### User Story 2 - Control Autonomy Level (Priority: P2)

A developer wants to control how aggressively the pipeline auto-fixes issues. They choose between `cautious` (stop at every finding), `balanced` (auto-fix clear issues, stop when uncertain), and `autopilot` (fix everything, stop only on genuine blockers).
...
**Independent Test**: Run `/spex:yolo --autonomy cautious` and verify it stops at each review finding. Run `/spex:yolo --autonomy autopilot` and verify it only stops on genuine blockers.
...
**Independent Test**: Run `/spex:yolo --no-coderabbit` and verify CodeRabbit is skipped during deep-review. Run `/spex:yolo --coderabbit` and verify CodeRabbit is explicitly included.

**Acceptance Scenarios**:

1. **Given** the user invokes `/spex:yolo --no-external`, **When** the pipeline reaches deep-review, **Then** only internal review perspectives run (no CodeRabbit, no Copilot).
2. **Given** the user invokes `/spex:yolo --coderabbit`, **When** CodeRabbit auth check fails, **Then** the pipeline stops at startup with a clear error message before any work begins.
3. **Given** the user invokes `/spex:yolo` without review flags, **Then** the deep-review trait's default configuration determines which external tools run.

---

### User Story 4 - Worktree Isolation (Priority: P3)

A developer with the worktrees trait enabled runs `/spex:yolo`. The specify stage creates a worktree as usual, and the entire remaining pipeline runs inside that worktree.

**Why this priority**: Worktree isolation is an existing trait behavior. Yolo must integrate correctly with it but does not change how worktrees work.
...
1. **Given** the worktrees trait is enabled, **When** `/spex:yolo` runs, **Then** specify creates the feature branch and worktree, and plan, tasks, implement, and review stages all execute inside the worktree.
2. **Given** the worktrees trait is disabled, **When** `/spex:yolo` runs, **Then** all stages execute in the current working directory on the feature branch.
...
1. **Given** the user invokes `/spex:yolo --create-pr`, **When** all pipeline stages succeed, **Then** a pull request is created for the feature branch.
2. **Given** the user invokes `/spex:yolo` without `--create-pr`, **When** all stages succeed, **Then** no PR is created and the user is informed of successful completion.
...
- **FR-004**: System MUST support three autonomy levels (`cautious`, `balanced`, `autopilot`) controlled via `--autonomy` flag, defaulting to `balanced`
...
- **FR-006**: System MUST auto-fix review findings according to the selected autonomy level and re-run the review (max 2 retry cycles per stage)
- **FR-007**: System MUST pause and present findings to the user when the AI is uncertain about the correct fix (in `balanced` mode) or when any finding is detected (in `cautious` mode)
- **FR-008**: System MUST automatically resume the pipeline after the user answers a blocking question
- **FR-009**: System MUST write current pipeline state to `.specify/.spex-yolo-phase` as JSON for status line integration
...
- **Pipeline State**: Tracks current stage, total stages, autonomy level, start time, retry count. Persisted to `.specify/.spex-yolo-phase` as JSON.
- **Autonomy Level**: One of `cautious`, `balanced`, `autopilot`. Determines when the pipeline pauses for human input.
...
- **SC-001**: A well-scoped feature (under 10 tasks) completes the full pipeline in `autopilot` mode without any human intervention
- **SC-002**: In `balanced` mode, the pipeline auto-fixes at least 80% of review findings without stopping
