# Review Summary: Autonomous Full-Cycle Workflow (spex:ship)
...
The `spex:ship` skill introduces a single entry point (`/spex:ship`) that chains the entire pipeline autonomously: specify, clarify, review-spec, plan, review-plan, tasks, implement, deep-review, and verify. The developer points it at a brainstorm document and chooses an ask level that controls how much human oversight the pipeline requires.

Three ask levels serve different situations. "Always" stops at every review finding for developers working on critical features or learning the workflow. "Smart" (the default) auto-fixes straightforward issues like formatting and style while pausing on architectural or ambiguous questions that need human judgment. "Never" fixes everything it can and only stops when genuinely blocked, suitable for well-scoped features after thorough brainstorming.

The skill tracks its progress in a JSON state file (`.specify/.spex-ship-phase`) that records the current stage, retry count, and ask level. This enables status line integration so developers can see pipeline progress at a glance.
...
- The oversight classification logic and retry mechanism are the most novel parts
- Check whether the three ask levels are well-defined enough to implement
...
|----------|-------------|
| `spec.md` | Defines the spex:ship skill with 5 user stories, 13 functional requirements, and 6 edge cases |
...
| `quickstart.md` | Usage examples for each ask level and flag combination |
| `REVIEWERS.md` | This file |

## Technical Decisions

### Single SKILL.md vs. Command + Skill Split
- **Chosen approach:** Single `spex/skills/ship/SKILL.md` containing all orchestration logic
- **Alternatives considered:**
  - Command file + skill file: Rejected because ship is not a speckit command. Overlays don't append to skills, so the command file would serve no purpose.
...
### Ship as a Skill, Not a Trait
- **Chosen approach:** `spex:ship` is a standalone skill, not registered as a trait in `spex-traits.json`
- **Alternatives considered:**
  - Register as trait: Rejected because traits modify existing command behavior via overlays. Ship is a new entry point that composes existing behaviors without modifying them.
- **Trade-off:** Ship is always available when the plugin is installed, no enable/disable needed. But it can't be discovered via `/spex:traits list`.

### Oversight Classification at Ship Level
- **Chosen approach:** The ship skill classifies review findings (unambiguous/ambiguous/blocker) rather than pushing this logic into individual review skills
- **Alternatives considered:**
  - Push autonomy into review skills: Rejected because it would require modifying 4+ existing skills and would couple them to the ship workflow.
...
| `spec.md` FR-009: State file format | The JSON schema for `.spex-ship-phase` is a new interface contract. Other tools (status line scripts) will depend on it. |
...
- [ ] The ask level definitions are specific enough to implement consistently
...
- [ ] The oversight classification ("unambiguous" vs "ambiguous") is subjective and may be inconsistent across runs
- [ ] State file cleanup: if the pipeline completes successfully but cleanup fails, stale state files may confuse future runs

## Scope Boundaries
- **In scope:** Single-session autonomous pipeline, 3 ask levels, state tracking, external tool flag pass-through, optional PR creation, `--resume` from interrupted pipelines, `--start-from` for partial runs
...
| Item | Name | Context |
|------|------|---------|
| Skill name | `spex:ship` | Follows `spex:` prefix convention. "ship" conveys end-to-end delivery from brainstorm to shippable code. |
| State file | `.specify/.spex-ship-phase` | Follows existing `.spex-phase` naming pattern, scoped to ship. |
| Autonomy flag | `--ask` | Full word, not abbreviated. Values: `always`, `smart`, `never`. |
...
```json
{
  ...
  "stage_index": "int (0-8)",
  "ask": "string (cautious|balanced|autopilot)",
...
|------|--------|------------|
| Context window exhaustion during 9-stage pipeline | High | Each stage delegates to existing skills which manage their own context. Large features may need `never` to minimize back-and-forth. |
| Inconsistent oversight classification across runs | Med | Classification criteria are documented in the skill. Users can override with `always` for critical features. |
...
| Existing skill behavior changes when invoked through ship | Med | Ship delegates to existing skills/commands unchanged. Acceptance test SC-005 explicitly verifies identical behavior. |
