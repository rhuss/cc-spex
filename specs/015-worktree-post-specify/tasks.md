...
- [x] T001 [US1] Update `spex/skills/worktree/SKILL.md` Step 4 (path computation) to use colon naming convention: derive `REPO_NAME` via `basename $(git rev-parse --show-toplevel)`, compute `WORKTREE_PATH` as `<resolved-base>/<repo-name>:<branch-name>`
- [x] T002 [US1] Update `spex/skills/worktree/SKILL.md` Step 5 (commit scope) to use `git add -A` instead of `git add "$SPEC_DIR"`, with a `git diff --cached --quiet` guard to skip empty commits
- [x] T003 [P] [US1] Update `spex/skills/worktree/SKILL.md` Step 8 (switch instructions) to show the colon-format path in the `cd` command
- [x] T004 [P] [US1] Update FR references throughout `spex/skills/worktree/SKILL.md` to match the new spec numbering (FR-001 through FR-012)
- [x] T005 [P] [US1] Verify `spex/overlays/worktrees/skills/speckit-specify/SKILL.append.md` correctly delegates to `{Skill: spex:worktree}` and stays under 30 lines (constitution II)
...
- [x] T006 [US2] Verify the existing `spex-initialized: false` sentinel detection in `context-hook.py` works correctly when the session starts in a worktree directory (`.git` is a file, not a directory)
- [x] T007 [P] [US2] Verify `.claude/skills/` is in `.gitignore` so it gets regenerated per session, not carried from the feature branch
...
- [x] T008 [US3] Update `spex/skills/worktree/SKILL.md` List action output to display colon-format paths (e.g., `cc-spex:004-user-auth` instead of `../004-user-auth`)
...
- [x] T009 [US4] Review `spex/skills/worktree/SKILL.md` Cleanup action for compatibility with colon-format paths in `git worktree remove` command
...
- [x] T010 Run `make release` to validate plugin schema and integration test
