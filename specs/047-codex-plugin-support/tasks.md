...
- [X] T051 [US4] Implement Codex native task-progress presentation with concise transcript fallback in `spex/scripts/adapters/codex/progress.py`
- [X] T052 [US4] Report and reconcile visible-progress/state disagreement during resume in `spex/extensions/spex/commands/speckit.spex.ship.md`
...
- [X] T060 [US5] Require orchestrator wait, specification review, reconciliation, dependent release, and partial-work preservation in `spex/extensions/spex-teams/commands/speckit.spex-teams.orchestrate.md`
...
- [X] T068 [US6] Make unresolved markers, unavailable commands, foreign paths, and identity collisions fatal during release in `spex/scripts/spex-validate-materialized.sh`
- [X] T069 [US6] Extend version synchronization and package inventories across both distributions in `Makefile`, `VERSION`, `spex/VERSION`, `spex/setup.yml`, and `spex/bundle.yml`
- [X] T070 [US6] Add `test-install-claude`, `test-install-codex`, `test-install-combined`, aggregate `test`, and pre-tag cross-harness gates to `Makefile`
- [X] T071 [US6] Update marketplace installation, adapter architecture, capability degradation, and coexistence documentation in `README.md`, `spex/docs/help.md`, `docs/design.md`, and `TESTING.md`
...
- [X] T074 Run all scenarios from `specs/047-codex-plugin-support/quickstart.md` and record evidence in `specs/047-codex-plugin-support/validation.md`
...
- [X] T076 Run the aggregate unit, lifecycle, recovery, materialization, and three installation suites through `make test` and document any intentional platform skips in `TESTING.md`
- [X] T077 Run the full pre-release validation path without tagging through the release-check target in `Makefile`
