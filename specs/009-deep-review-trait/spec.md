...
### User Story 4 - External Review Tool Integration (Priority: P3)

When external review CLIs are installed on the user's system, the deep-review trait includes them as additional review perspectives alongside the internal agents. Two external tools are supported: CodeRabbit CLI and GitHub Copilot CLI. Each provides an independent AI review from a different model, catching blind spots that Claude-based agents might miss. Both are optional; if neither is installed, the trait works with internal agents only (graceful degradation).

**Why this priority**: External review adds value but is optional. The trait must work fully without external dependencies.

**Independent Test**: Can be tested by installing one or both external CLIs, enabling the `deep-review` trait, running a review, and verifying external findings are included in the merged results.

**Acceptance Scenarios**:

1. **Given** the CodeRabbit CLI is installed, **When** Stage 2 review starts, **Then** CodeRabbit reviews uncommitted changes and its findings are parsed and merged into the consolidated findings.
2. **Given** the Copilot CLI is installed, **When** Stage 2 review starts, **Then** Copilot reviews the changes and its findings are parsed and merged into the consolidated findings.
3. **Given** neither external CLI is installed, **When** Stage 2 review starts, **Then** only internal agents run without error or warning about missing CLIs.
4. **Given** an external tool and an internal agent find the same issue, **When** findings are merged, **Then** the duplicate is consolidated into a single finding with both sources noted.
5. **Given** both CodeRabbit and Copilot are installed, **When** Stage 2 review starts, **Then** both run and their findings are merged alongside internal agent findings.
...
- What happens when an external review tool (CodeRabbit or Copilot CLI) times out or returns an error? The error is logged in `review-findings.md` but does not block the review. Internal agent findings and any other external tool findings are still processed normally.
...
- What happens when one review agent fails or times out? The failed agent is skipped, the failure is logged in `review-findings.md`, and the review proceeds with findings from the remaining agents (graceful degradation, consistent with CodeRabbit failure handling).

### Out of Scope

- **Custom agent definitions**: Users cannot define, add, or modify review agents. The five internal agents are fixed.
- **PR-level review**: No GitHub PR comment integration, no posting findings as PR review comments.
- **Runtime/dynamic analysis**: Review is static analysis only (reading code). No execution, profiling, or runtime instrumentation.
- **Non-code artifact review**: The trait reviews source code files only, not documentation, images, or configuration templates.

## Clarifications

### Session 2026-03-28

- Q: What should happen when one review agent fails or times out while others succeed? → A: Graceful degradation: skip failed agent, proceed with remaining findings, note gap in report.
- Q: When a user runs `/spex:review-code` multiple times, should `review-findings.md` reset or accumulate? → A: Replace: each run overwrites with fresh findings (git preserves history).
- Q: Should the system report progress to the user during the review process? → A: Per-agent status: report when each agent starts and finishes (e.g., "Agent 3/5: Security... done, 2 findings").
- Q: What is explicitly out of scope for the deep-review trait? → A: Custom agent definitions, PR-level review (GitHub comments), runtime/dynamic analysis, non-code artifact review.
- Q: Should the fix loop auto-fix findings silently, or present proposed fixes for user approval? → A: Fully autonomous: apply fixes silently, user reviews all changes after completion via git diff.
...
- **FR-014**: When Stage 1 fails (compliance score < 95%), the system MUST report the compliance score, list the non-compliant requirements, and instruct the user to resolve spec compliance issues before deep review can proceed. Stage 2 MUST NOT start.
- **FR-013**: "Changed files" MUST be defined as the union of: (a) files modified between the main branch and HEAD (`git diff <main-branch>...HEAD`), and (b) any uncommitted changes in the working tree. During fix loop re-review rounds, "changed files" narrows to only the files modified by the most recent fix round.
...
- **FR-021**: Each review agent prompt MUST include:
  ...
  - Confidence scoring (0-100 scale, only report findings with confidence >= 70, except Critical findings which MUST be reported at confidence >= 50)
...
- **FR-024**: If a review agent fails or times out, the system MUST skip that agent, log the failure in `review-findings.md` (agent name, error reason), and proceed with findings from the remaining agents. A single agent failure MUST NOT block or invalidate the overall review.

- **FR-023**: Review agents MUST be language-aware: they adapt their checklists based on the programming languages detected in the changed files. For mixed-language changes (e.g., Go + YAML + Dockerfile), agents MUST apply language-specific checks for each language present, combining relevant checklist items.
...
- **FR-031**: System MUST merge findings from all agents (internal and external) into a single consolidated list, deduplicating by file path + line range + issue category. Issue categories correspond to the review agent focus areas: correctness, architecture, security, production-readiness, test-quality, and external (used for both CodeRabbit and Copilot findings). When two findings match on file + overlapping line range + same category, the finding with more detail is kept and the other source is noted.
- **FR-032**: System MUST write a `review-findings.md` artifact to the spec directory with the following structure: review metadata (date, branch, rounds executed, gate outcome), per-round sections listing each finding with severity, confidence, file:line, description, source agent, and resolution status (fixed/remaining/not-applicable), and a summary with counts by severity and overall gate result. Each review run MUST overwrite the previous `review-findings.md` (git history preserves prior versions).

#### Autonomous Fix Loop

- **FR-040**: System MUST auto-fix Critical and Important findings and re-review changed files (per FR-013 re-review scope) after each fix round.
...
- **FR-044**: Gate behavior MUST vary by invocation context:
  - **Superpowers context** (triggered as a quality gate): gate pass allows proceeding to verification; gate fail blocks completion.
  - **Manual context** (user runs `/spex:review-code` directly): gate pass/fail is advisory. Findings are reported and the user decides whether to proceed. The review does NOT block further commands.
- **FR-045**: Fixes MUST be performed by the main conversation agent (not by review agents). The fix agent receives the consolidated findings list and applies changes autonomously without per-fix or per-round user approval. After each fix round, changes MUST be staged before re-review begins. The user reviews all accumulated changes after the loop completes (via git diff or equivalent).
- **FR-046**: When multiple findings target the same file, fixes MUST be applied sequentially within that file (top-to-bottom by line number) to prevent conflicts from overlapping edits.
- **FR-047**: The entire review-and-fix cycle (all rounds) MUST execute within the same conversation context. Review agents operate in isolated sub-contexts (per FR-022), but the fix agent and orchestration logic run in the main conversation.
...
#### External Review Tool Integration

- **FR-060**: The system MUST detect available external review CLIs at the start of Stage 2. Two tools are supported: CodeRabbit CLI and GitHub Copilot CLI.
- **FR-061**: When the CodeRabbit CLI is available, it MUST be invoked to review uncommitted local changes in a parseable output mode, producing findings that can be merged.
- **FR-064**: When the Copilot CLI is available, it MUST be invoked to review the current changes, with output captured in a parseable format. Since Copilot CLI lacks structured output flags, the invocation prompt MUST request findings in a structured format (severity, file, line, description).
- **FR-062**: When an external CLI is NOT installed, the system MUST proceed without it. If no external tools are available, only internal agents run. No error or warning is shown for missing optional tools.
- **FR-063**: External tool findings MUST be parsed and merged into the consolidated findings list alongside internal agent findings, using the same deduplication rules (FR-031).
- **FR-065**: Both external tools MAY run in the same review session. Their findings are independently parsed and merged. Deduplication applies across all sources (internal agents, CodeRabbit, Copilot).

#### External Tool Opt-In/Opt-Out

- **FR-066**: The system MUST support a default configuration for external tool usage via the `external_tools` key in `.specify/spex-traits.json`. When no configuration is set, external tools are **enabled by default** (opt-in).
- **FR-067**: `/spex:review-code` MUST support flags to override the default:
  - When default is enabled: `--no-external` (disable all), `--no-coderabbit` (disable CodeRabbit only), `--no-copilot` (disable Copilot only)
  - When default is disabled: `--external` (enable all), `--coderabbit` (enable CodeRabbit only), `--copilot` (enable Copilot only)
- **FR-068**: Flag parsing MUST be applied before hint text extraction. Flags are consumed and not passed as hint text to review agents.

#### Manual Invocation with Hints

- **FR-070**: When `/spex:review-code` is invoked with argument text (after flag extraction per FR-068), the remaining argument text MUST be injected as additional review focus into each review agent's prompt.
- **FR-071**: Hint injection MUST supplement (not replace) each agent's standard checklist and focus area.

#### Progress Reporting

- **FR-090**: The system MUST report progress to the user at each stage transition and for each review agent. Status updates MUST include: Stage 1/2 start and completion, each agent's name and sequence number when it starts and finishes (e.g., "Agent 3/5: Security... done, 2 findings"), each fix round start and completion, and the final gate outcome.
- **FR-091**: In parallel mode (teams trait), progress updates MUST still be reported per-agent as each completes, even though agents run concurrently.
...
- **Review Agent**: A specialized sub-agent with a focused review perspective, its own prompt template, and isolated context. Five internal agents defined (correctness, architecture, security, production-readiness, test-quality). Two optional external tools (CodeRabbit CLI, Copilot CLI) provide additional perspectives.
...
### Measurable Outcomes (verifiable at implementation time)
...
### Post-Launch Validation Targets (require empirical data over time)
...
- CodeRabbit CLI, if used, requires the user to have an account and API key configured. The free tier (unlimited repos, rate-limited) is sufficient for most single-feature reviews.
- Copilot CLI, if used, requires a GitHub Copilot subscription (free tier: 50 premium requests/month shared across all Copilot usage). Each review invocation consumes premium requests from this pool.
- Each round's fixes are staged before re-review (see FR-045 for the formal requirement). The conversation context constraint is captured in FR-047.
