...
Reviewing a 500-line specification document from scratch is daunting. `REVIEWERS.md` is generated by spex to make even medium to large PRs reviewable within 30 minutes. It follows a "general to specific" structure: motivation first (Why This Change), then a one-paragraph summary (What Changes), then the design decisions that need human judgment, then areas of concern, and finally the detailed scope lists as reference material.
