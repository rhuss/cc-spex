...
- If `ask` is `"smart"` or `"never"`: suppress all user prompts (do NOT prompt the user interactively), complete the verification autonomously, and return immediately so the pipeline can advance.
