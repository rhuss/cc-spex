...
|----------------|---------|--------------|
| Rough idea | `/speckit-spex-brainstorm` | Collaborative dialogue refines idea into formal spec |
...
|-----------|---------|--------------|
| Spec/code mismatch | `/speckit-spex-evolve` | AI analyzes, recommends update spec vs fix code |
...
|----------|----------|-----|
| Vague idea | Clear spec | `/speckit-spex-brainstorm` |
| Clear requirements | Formal spec | `/speckit-specify` |
| Validated spec | Working code | `/speckit-implement` |
| Spec + code mismatch | Alignment | `/speckit-spex-evolve` |
| Draft spec | Validation | `/speckit-spex-review-spec` |
| Code changes | Compliance check | `/speckit-spex-review-code` |
...
```
SPEC CREATION
  /speckit-spex-brainstorm    Rough idea → spec (interactive)
  /speckit-specify          Clear reqs → spec (direct)
  /speckit-constitution  Project-wide principles

VALIDATION
  /speckit-spex-review-spec   Check spec quality
...
EVOLUTION
  /speckit-spex-evolve        Fix spec/code drift
```
...
Extensions add automated quality gates and integrations to the spec-kit workflow via lifecycle hooks. They inject checks into spec-kit commands so you don't have to remember to run them manually.

**Available extensions:**

| Extension | Purpose |
|-----------|---------|
| `spex-gates` | Automated spec reviews, plan reviews, spec PR creation |
...
Without extensions, you run each step manually:
```
/speckit-specify  →  /speckit-spex-review-spec  →  /speckit-plan  →  /speckit-spex-review-plan
```

With the spex-gates extension, reviews happen automatically via hooks:
```
/speckit-specify  →  (review runs automatically via hooks)
/speckit-plan     →  (spec review before, plan review + tasks + spec PR after)
```

Extensions are optional. Start without them to learn the workflow, then enable them when you want automated discipline.
...
- Ignore drift warnings (use `/speckit-spex-evolve`)
...
- Use `/speckit-spex-evolve` when reality differs from plan
...
2. Run `/speckit-spex-brainstorm` to turn it into a spec
