...
**`--resume` and `--start-from` are mutually exclusive** with each other and with the brainstorm file argument. If both `--resume` and `--start-from` are provided, fail with:
```
ERROR: Cannot use both --resume and --start-from. Choose one.
```

If `--resume` or `--start-from` is set alongside a brainstorm file, fail with:
```
ERROR: Cannot specify a brainstorm file with --resume or --start-from.
```
...
```bash
cat > .specify/.spex-yolo-phase << 'STATEEOF'
{
...
3. Calculate the next stage index: `NEXT_INDEX=$((LAST_INDEX + 1))`

4. If `NEXT_INDEX >= 9`, the pipeline was already complete. Report this and clean up.

5. Resume execution from `NEXT_INDEX`, preserving the original autonomy level and brainstorm file from the state.

6. Update the state file with `status: running` before proceeding.
...
3. Create the PR:
   ```bash
   FEATURE_NAME=$(head -1 specs/*/spec.md | sed 's/^# Feature Specification: //')
   SPEC_DIR=$(fd -t d -d 1 . specs/ | head -1)
   ...
   Generated by \`/spex:yolo\` in $(echo $AUTONOMY) mode.
